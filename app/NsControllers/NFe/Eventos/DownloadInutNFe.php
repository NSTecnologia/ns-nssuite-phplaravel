...
class DownloadInutNFe{
    ...
    public function downloadInut($conteudo, $caminho){
        $genericos = new Genericos;
        
        $conteudo = json_encode($conteudo, JSON_UNESCAPED_UNICODE);

        $genericos->gravarLinhaLog('[DOWNLOAD_INUT_NFE_INICIO]');
        $genericos->gravarLinhaLog('[DADOS_ENVIADOS]');
        $genericos->gravarLinhaLog($conteudo);

        $ret = $genericos->enviarConteudoParaAPI($conteudo, 'application/json', 'https://nfe.ns.eti.br/nfe/get/inut');
        ...
        if ($ret['status'] == 200){
            $nome = $ret['retInut']['chave'] . '-inutNFe';
        ...
        $genericos->gravarLinhaLog('[DOWNLOAD_INUT_NFE_FIM]');
